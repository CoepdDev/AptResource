...
namespace AptEMS.DAL
{
    public class FirstRound
    {
        ...
        public int AddFirstRound(Models.FirstRound e1)
        {

            con.Open();

            using (SqlCommand checkCmd = new SqlCommand("SELECT COUNT(*) FROM dbo.FirstRound WHERE Email = @Email", con))
            {
            ...
            SqlCommand cmd = new SqlCommand("sp_InsertFirstRound", con);
        ...
        public int AddNextRound(Models.SecondRound e1)
        {
        ...
        public int DeleteFirstRound(Models.FirstRound e1)
        {

            con.Open();

            SqlCommand cmd = new SqlCommand("sp_DeleteFirstRound", con);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public int UpdateFirstRound(Models.FirstRound e1)
        {

            con.Open();

            SqlCommand cmd = new SqlCommand("sp_UpdateFirstRound", con);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public List<Models.FirstRound> GetFirstRound()
        {
            List<Models.FirstRound> li = new List<Models.FirstRound>();


            con.Open();
            SqlCommand cmd = new SqlCommand("sp_GetFirstRound", con);
        ...
        public Models.FirstRound SearchFirstRound(Models.FirstRound e1)
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("sp_SearchFirstRound", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@Mobile", e1.Mobile);
            SqlDataReader dr = cmd.ExecuteReader();
            if (dr.HasRows)
            {
                if (dr.Read())
                {
                    ...
                    e1.Role = dr["Role"].ToString();
                    e1.InterviewerName = dr["InterviewerName"].ToString();
                    e1.Date = DateTime.Parse(dr["Date"].ToString());
                    e1.Remark = dr["Remark"].ToString();
                    e1.Email = dr["Email"].ToString();
